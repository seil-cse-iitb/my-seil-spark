...
public class PrevYearJoinWithCurrentSlot {
...
//        Dataset<Row> rows = sparkSession.read().jdbc(ConfigHandler.MYSQL_URL, tableName, properties);
        Dataset<Row> rows = sparkSession.read().format("jdbc")
                .option("url",ConfigHandler.MYSQL_URL)
                .option("dbtable",tableName)
                .option("user",ConfigHandler.MYSQL_USERNAME)
                .option("password",ConfigHandler.MYSQL_PASSWORD)
...
//        lines.createOrReplaceTempView("data");
        ...
        newData = newData.withColumn("secondsYear", functions.lit(31536000));
        ...
        Dataset<Row> resu =newData.groupBy(functions.window(timestamp1,"1 hour","1 minute")).avg("W").orderBy("window");

        Column windowstart_s = resu.col("window.start").cast("timestamp").cast("long");
        resu=resu.withColumn("windowstart_s",windowstart_s);
        resu = resu.withColumn("secondsYear", functions.lit(31536000));
        ...
        System.out.println("res\n"+res.queryExecution().sparkPlan().toString());
...
//        newData=newData.withColumn("WindowStart", functions.from_unixtime(functions.col("sToDate").minus(functions.col("sToDate").mod(60))));
...
//        newData=newData.groupBy("WindowStart").avg("W").withColumnRenamed("avg(W)", "Stream Average W");


//        newData.createOrReplaceTempView("newdata");


//        String query="select * from newdata order by WindowStart";
//        Dataset<Row> finalData = sqlContext.sql(query);
//        Dataset<Row> finalData = newData.orderBy("WindowStart");


//        finalData=finalData.join(res,"WindowStart").select("WindowStart","Stream Average W","Archival Average W","sensor_id");
